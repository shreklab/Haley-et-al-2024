...
borderAmp10 = borderAmp(strcmp(GID.expName,'foragingConcentration') & ...
    GID.lawnOD600 == 10 & GID.lawnVolume == 0.5);
...
%% Figure 3B - Concentrations tested w/ effective OD600 (Mini)
...
xticks(xValues); xticklabels({'0','0.5','1','5','10','1'})
...
exportgraphics(gca,[saveDir,'fig3b.pdf'])
...
summaryTableB = table({'0','0.5','1','5','10','1'}',...
    [ones(5,1);48],relativeBorder,relativeBorder_std,conditionLabels,colorValue,...
    'VariableNames',{'OD600','growth time (hr)','mean relative density','std relative density','label','saturation'})
summaryNoteB = {'* relative density was estimated as described in methods and Figure 2 - supplement 1-3';...
...
writeSourceData([saveDir,'fig3_data2.xlsx'],{'Figure 3 - Source Data 2';'Figure 3B - Relative density of small (0.5 μL) bacterial patches'},...
    {'Summary Statistics'},{summaryTableB;},{summaryNoteB})

%% Figure 3E - Example Traces (Mini)

wormNum = 214;
% plotTracks(info,data,wormNum,'timeOffset',saveDir,bodyPart,'yes');

%% Figure 3G - Encounters labeled for all worms (Mini)
...
exportgraphics(gcf,[saveDir,'fig3g.pdf'],'ContentType','vector')
...
sourceTableG = table(conditionLabels(conditionList),wormList,encounterList,...
...
sourceNoteG = {'* posterior probability of classification as explore or exploit as estimated by the Gaussian Mixture Model described in methods, Figure 2H, and Figure 2 - supplement 6';...
...
writeSourceData([saveDir,'fig3_data4.xlsx'],{'Figure 3 - Source Data 4';'Figure 3G - Encounter classification as explore or exploit and sense or non-sense'},...
    {'Summary Statistics';'Source Data'},{summaryTableF;sourceTableG},{[];sourceNoteG})

%% Figure 3I - Time before an exploit (mini)
...
exportgraphics(gca,[saveDir,'fig3i.pdf'])
...
% Source data
summaryTableI = table(conditionLabels,{'0','0.5','1','5','10','1'}',...
    [ones(5,1);48],relativeBorder,...
...
summaryNoteI = {'* mean of mean time before first exploitation (min) for each animal';...
...
sigmoidTableI = table(f.a,f.b,f.c,f.d,g.rsquare,'VariableNames',...
    {'a','b','c','d','r-squared'});
sourceTableI = table((1:numWorms)',conditionLabels(wormGroupInd),timeExploit./60,~doesExploit,...
    'VariableNames',{'worm #','label','time before first exploit (min)','only explores'})
sourceNoteI = {'* time (min) before first exploitation (or last observed time point) for each worm';...
    '* animals that only explored (i.e. p(exploit) < 0.5 for all encounters)'};
writeSourceData([saveDir,'fig3_data6.xlsx'],{'Figure 3 - Source Data 6';'Figure 3I - Time before first exploitation'},...
    {'Summary Statistics';'Sigmoid (y = d + (a-d)/(1 + (x/c)^b)';'Source Data'},{summaryTableI;sigmoidTableI;sourceTableI},{summaryNoteI;[];sourceNoteI})
%%
figure; hold on
colorInd = wormGroupInd+(numGroups.*doesExploit);
colorArray = [max(min(exploreColor + (1 - exploreColor).*(colorValue-0.1),1),0);...
...
% x = logspace(log10(borderAmp0)-1,log10(max(relativeBorder))+1,100);
% plot(x,f(log10(x) - log10(borderAmp0) + 1),'k','LineWidth',1)
xscale('log'); xlim([0.005 1000]); ylim([0 30]); yticks(0:10:30)
xticks([0.01 0.1 1 10 100]); xticklabels([0 0.1 1 10 100]); legend('off')
set(gca,'Units','inches','FontName','Arial','FontSize',8,'Box','on','TickDir','out'); 
